...
use crate::{PostFrameData, VIEW_TYPE, XrPose};
...
impl XrInput {
    ...
    pub(crate) fn post_frame(
        ...
        if let Some(left_hand) = &left_hand {
            *self.left_hand.lock() = left_hand.clone();
        }

        if let Some(right_hand) = &left_hand {
            *self.right_hand.lock() = right_hand.clone();
        }

        *self.views.lock() = views.iter().map(|f| openxr_pose_to_glam(&f.pose)).collect();
